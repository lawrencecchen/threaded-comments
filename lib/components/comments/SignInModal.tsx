...
function MagicLinkView({ email, setEmail, setAuthView }: MagicLinkViewProps): JSX.Element {
  ...
  return (
    <>
      <h1 className="text-2xl leading-6 font-semibold" id="modal-headline">
        Sign in
      </h1>
      <p className="text-sm my-2 text-gray-600">Don&apos;t worry, we&apos;ll save your comment.</p>
      <div>
        <div>
          <button
          ...
          >
            ...
            <span className="ml-2 text-sm leading-none">Sign in with GitHub</span>
          </button>
        </div>
        <div className="flex items-center my-3">
          <div className="w-full h-px bg-gray-300"></div>
          <div className="text-gray-600 text-sm px-2 flex-shrink-0">Or, use my email address</div>
          <div className="w-full h-px bg-gray-300"></div>
        </div>
        <form onSubmit={handleSubmit}>
          <label className="block mb-2">
            ...
            <input
              ...
              value={email}
              autoComplete="email"
              className="mt-1 p-2 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border border-gray-300 rounded-md transition duration-75"
...
function AwaitingConfirmationView({
  ...
  return (
    <div className="flex flex-col items-center">
      <h1 className="text-2xl leading-6 font-semibold text-gray-900" id="modal-headline">
        Confirm your email
      </h1>
    ...
    <Transition show={show}>
      <div className="fixed z-50 inset-0 overflow-y-auto">
        <div className="flex relative items-end justify-center min-h-screen pt-4 px-4 pb-4 text-center sm:items-center sm:p-0">
          ...
          <div
            className="w-full inline-block bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-sm"
            role="dialog"
            aria-modal="true"
            ...
            >
              ...
              <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                ...
                <button
                  type="button"
                  onClick={() => close('signInModal')}
                  className="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus-ring sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm"
                >
                  Cancel
                </button>
              </div>
